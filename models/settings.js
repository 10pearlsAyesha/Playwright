...
class SettingsPage {
  constructor(page) {
    ...
    this.languageDropdown = page.locator("//div[@class='v-select__selections']//div[@class='row']//div[2]//span");
    this.selectAnotherLanguage = page.locator("//div[@id='list-item-887-0']//span[contains(text(),'" + `${process.env.another-platform-language}` + "')]");
    this.selectDefaultLanguage = page.locator("//div[@id='list-item-887-0']//span[contains(text(),'" + `${process.env.default-platform-language}` + "')]");
  ...
  async platformLanguage() {
    ...
    const selectedLanguage = await profilePage.languageDropdown.textContent();
    expect(selectedLanguage).toBe(`${process.env.another-platform-language}`);
